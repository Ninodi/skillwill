...
function MainPage() {
    ...
    const {sendRequest} = useRequest({url: 'https://crudapi.co.uk/api/v1/testTodos', method: 'POST'})
    const setTodo = (text, username, date) => {
      let newTodo = {
        text,
        username,
        date,
    ...
    return (
      <div>
        ...
        <div>
          {todos.map(todo => (
            <Todo key={todo.id} todoId={todo.id} todoUser={todo.username} todoDate={todo.date} todoText={todo.text} todoStatus={todo.isCompleted} markAsDone={() => markAsDone(todo.id)} deleteTodo={() => deleteTodo(todo.id)}/>
          ))}
        </div>
      </div>
    );
